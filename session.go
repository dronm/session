...
var logLevels = []string{"ERROR", "WARN", "DEBUG"}
...
type Session interface {
	...
	Put(key string, value any) error //set session value and flushes
	Get(key string, value any) error //get session value
...
func NewManager(providerName string, maxLifeTime int64, maxIdleTime int64, sessionsKillTime string, provParams ...any) (*Manager, error) {
...
func (manager *Manager) InitProvider(provParams []any) error {
...
func parseTime(timeStr string) (time.Time, error) {
	lay_out := ""
	if len(timeStr) == len(timeSecLayout) {
		lay_out = timeSecLayout
	} else if len(timeStr) == len(timeMinLayout) {
		lay_out = timeMinLayout
	} else {
