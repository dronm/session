...
func (pder *Provider) SessionInit(sid string) (session.Session, error) {
	...
	sess := &SessionStore{sid: sid}
	// tm := time.Now()
	// if err := pder.setValue(sid, "time_created", tm); err != nil {
	// 	return nil, fmt.Errorf("setValue() time_created failed: %v", err)
	// }
	// if err := pder.setValue(sid, "time_accessed", tm); err != nil {
	// 	return nil, fmt.Errorf("setValue() time_accessed failed: %v", err)
	// }
...
func (pder *Provider) SessionRead(sid string) (session.Session, error) {
	// var tm time.Time
	// if err := pder.getValue(sid, "time_created", &tm); err != nil && errors.Is(err, ErrKeyNotFound) {
...
func (pder *Provider) sessionAccessed(sid string) error {
	lastAccessKey := pder.getPrefixedKey(sid, "time_accessed")
	val, err := pder.client.Get(context.Background(), lastAccessKey).Time()
	if err == nil && time.Since(val) < time.Minute {
...
func (pder *Provider) getValue(sid, key string, t any) error {
...
func (pder *Provider) getValueForKey(redisKey string, t any) error {
	val_b, err := pder.client.Get(context.Background(), redisKey).Bytes()
	if err != nil {
		return err
	}
	if len(val_b) == 0 {
		return ErrKeyNotFound // no value found
	}
...
func (pder *Provider) setValue(sid string, key string, val any) error {
