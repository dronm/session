// testing functions for session/pg.
package pg
...
func NewTestValues() map[string]any {
...
func assertNoValues(t *testing.T, currentSession session.Session, tests map[string]any) {
	for key, wanted := range tests {
		ptr := reflect.New(reflect.TypeOf(wanted))
		err := currentSession.Get(key, ptr.Interface())
		if err == nil {
...
func NewManager(t *testing.T, idleTime int64, lifeTime int64, killTime string) (*session.Manager, error) {
	dbpool, err := pgxpool.New(context.Background(), getTestVar(t, ENV_PG_CONN))
	if err != nil {
		t.Fatalf("NewManager() failed(): %v", err)
	}
	return session.NewManager(PROVIDER, idleTime, lifeTime, killTime, dbpool, "54ref65w104erc6f4c69e14cfv")
}
